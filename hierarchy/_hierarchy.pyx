...
cpdef np.ndarray[np.float64_t, ndim=1] ward_condensed_pdist(
    np.ndarray[np.float64_t, ndim=2] centers,
    np.ndarray[np.float64_t, ndim=1] sizes
):
    cdef int n = centers.shape[0]
    cdef int d = centers.shape[1]
    cdef Py_ssize_t L = n * (n - 1) // 2
    cdef np.ndarray[np.float64_t, ndim=1] pdist = np.empty(L, dtype=np.float64)

    cdef int i, j, k
    cdef double diff, sqdist, ni, nj, w
    cdef np.npy_int64 idx
